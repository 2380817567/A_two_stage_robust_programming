...
model = gp.Model('A_two-stage_robust_programming')
...
#变动部分 增加了
worst_balancing_cost = model.addVar(lb=-5000,name='worst_cost')
#设置目标(改动）
day_ahead_cost = gp.quicksum(C[p]*P[p]+CRU[p]*RU[p]+CRD[p]*RD[p] for p in Ps)
model.setObjective(day_ahead_cost+worst_balancing_cost,GRB.MINIMIZE)

#添加约束(加了下面两行）
model.addConstr(worst_balancing_cost >= gp.quicksum(C[p]*rh[p] for p in Ps)+200*(gp.quicksum(Lhshed[bus] for bus in buses)),name='high_scenario_cost')
model.addConstr(worst_balancing_cost >= gp.quicksum(C[p]*rl[p] for p in Ps)+200*(gp.quicksum(Llshed[bus] for bus in buses)),name='low_scenario_cost')

...
vertice, deltaWs1, deltaWs2 = gp.multidict ({
    'A':[10,8],'B':[4,20],'C':[-4,20],'D':[-10,8],
    'E':[-10,-8],'F':[-4,-20],'G':[4,-20],'H':[10,-8]
})
...
R1D = model.addVar(name='R1')
R2D = model.addVar(name='R2')
R3D = model.addVar(name='R3')
...
#鲁棒的转化，只需要满足不确定支撑集的顶点即可
model.addConstrs((worst_cost>=costb[v] for v in vertice),name='ribust_beta')
...
model.addConstr(P3-R3D>=0,name='day_ahead_unit3_down_constraint')  #这里up是不是打错了，应该是3啊？
                                                                   #我改完了好像也不影响结果，那我就给改成3了，应该是up打错了
